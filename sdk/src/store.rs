...
impl Store {
    ...
    fn create_bmff_data_hash_with_exclusions(alg: &str) -> BmffHash {
    ...
    fn generate_bmff_data_hashes_from_stream(
        input: &mut dyn CAIRead,
        alg: &str,
        calc_hashes: bool,
    ) -> Result<Vec<BmffHash>> {
        let mut hashes: Vec<BmffHash> = Vec::new();
        let mut dh = Self::create_bmff_data_hash_with_exclusions(alg);
        if calc_hashes {
            dh.gen_hash_from_stream(input)?;
        } else {
            match alg {
                "sha256" => dh.set_hash([0u8; 32].to_vec()),
                "sha384" => dh.set_hash([0u8; 48].to_vec()),
    ...
    fn start_save_stream(
        ...
        if is_bmff {
            ...
            if !pc.update_manifest() {
                let bmff_hashes = Store::generate_bmff_data_hashes_from_stream(
                    &mut intermediate_stream,
                    pc.alg(),
                    false,
                )?;
